...
| 3 | WS @18443, `X-Test-DN` (flo DN) | `curl -sk -i -N --http1.1 --max-time 3 -H 'Connection: Upgrade' -H 'Upgrade: websocket' -H 'Sec-WebSocket-Version: 13' -H 'Sec-WebSocket-Key: x3JJHMbDL1EzLkh9GBhXDw==' -H 'X-Test-DN: emailAddress=flo@example.test,CN=Flo Lou,OU=Admins,O=Example Org' http://127.0.0.1:18443/api/v4/websocket` (plain HTTP — see the 18443 exception above) | `101` (127.0.0.1 test seam) |
