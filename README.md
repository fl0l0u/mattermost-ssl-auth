...
* The proxy normalizes the forwarded WebSocket `Origin` to the SiteURL, so browser WebSockets work on the loopback test ingress too — Mattermost compares the Origin to the SiteURL with the port literal, so a browser on a non-default port would otherwise be rejected (in production the normalization is a no-op: the browser Origin already equals the SiteURL).
